...
def btn_prev_clicked():
    global curr_index   # refer to the global variable curr_index
    # decrease curr_index by 1, using modulo to go back to the end if curr_index is at 0
    curr_index = (curr_index - 1) % lst_products.size()
    # update form fields with product at curr_index
    update_form(curr_index)
...
def update_form(curr_index):
    ...
    change_text(txt_name, p.name)
    # TODO: update text of txt_price
    change_text(txt_price, str(p.price))
    
    # TODO: update text of txt_stock
    change_text(txt_stock, str(p.stock))
...
def btn_load_clicked():
    for p in load_products():
        ...
        # add product to listbox
        lst_products.insert(END, p.name)
    messagebox.showinfo("Loaded", f"Loaded {lst_products.size()} products.")

def btn_save_clicked():
    save_products(fpt_store.clone_products())
    messagebox.showinfo("Saved", f"Saved {lst_products.size()} products.")
